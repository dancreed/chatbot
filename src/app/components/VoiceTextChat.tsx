    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col">
        ...
        <div className="chat-window w-full bg-white text-black p-6 rounded-lg mb-6 shadow-lg max-h-[40vh] overflow-y-auto">
          {messages.map((m, i) => (
            <div key={i} className={`mb-4 ${m.sender === "user" ? "text-right" : "text-left"}`}>
              <span className={`inline-block px-4 py-2 rounded-xl ${m.sender === "user" ? "bg-blue-600 text-white" : "bg-gray-200 text-black"}`}>
                {m.text}
              </span>
            </div>
          ))}
      ...
      <div className="w-full bg-white border-t-2 border-gray-300 fixed left-0 right-0 bottom-0 z-50 flex justify-center py-5">
        <div className="flex w-full max-w-2xl gap-2 px-4">
          ...
          <button
            className={`rounded-full border-2 ml-2 w-12 h-12 flex items-center justify-center ${listening ? "bg-orange-400 border-orange-600" : "bg-gray-200 border-blue-600"}`}
